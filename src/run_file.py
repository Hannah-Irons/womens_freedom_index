...
The data available isn't the raw data from the Freedom 
project, and is made up of the econmic and personal freedom factors that are all on a scored
scale between 0-10. The Human Freedom report offers some insight in the meanings behind 
these factors but that their definitions, or what the score directly translates to numerically.
...
# retrieve loaction path and change directory if necessary. 
# print(os.getcwd())
...
# Because of the processed nature of the data set I want to drop some of the factors that are generated by the more base factors.
df = df.drop(['ISO_code', 'ef_score','pf_score', 'pf_rank', 'ef_rank', 'hf_rank', 'hf_quartile'], axis=1)

# print(df.dtypes)
# The whole dataset come in as "objects" despite the mojority of the data being a numerical score. 
